...
const BmiCalculator = () => {
  const updatedHeight = JSON.parse(localStorage.getItem('height'))
  const updatedWeight = JSON.parse(localStorage.getItem('weight'))
  const [height, setHeight] = useState(
    updatedHeight !== null ? updatedHeight : 170,
  )
  const [weight, setWeight] = useState(
    updatedWeight !== null ? updatedWeight : 60,
  )
